...
function outMv = EncodeEGaVector(scalarArray)
    ...
    if (length(size1) == 2 && size1(1) == 1 && size1(2) == 1)
        outMv = ga21.Multivector(1, [0; 0; double(scalarArray')]);

        return;
    end

    if (length(size1) ~= 2 || size1(1) ~= 1)
        error('Incorrect input size');
    end
    
    outMv = ga21.Multivector(1, [zeros(2, size1(2)); x]);
end
