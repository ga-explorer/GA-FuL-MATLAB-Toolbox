function latexVGA(x, msg)
    % latexVGA  Extended LaTeX display function for MATLAB/.NET objects
    %
    % Usage:
    %   latexVGA(x)
    %   latexVGA(x, msg)   % msg is a string shown alongside x
    ...
    if nargin == 2 && (isstring(msg) || ischar(msg))
        ...
        if isa(x, 'GeometricAlgebraFulcrumLib.Algebra.GeometricAlgebra.Float64.Multivectors.XGaFloat64Multivector')
            try
                str = char(GeometricAlgebraFulcrumLib.Modeling.Utilities.Text.TextUtils.VGaToLaTeX(x));
                fprintf('%s\n', str);
            catch
