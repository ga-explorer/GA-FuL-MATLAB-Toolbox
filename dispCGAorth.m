function dispCGAorth(x, msg)
    % dispCGAorth  Extended text display function for MATLAB/.NET objects
    %
    % Usage:
    %   dispCGAorth(x)
    %   dispCGAorth(x, msg)   % msg is a string shown alongside x
    ...
    if nargin == 2 && (isstring(msg) || ischar(msg))
        ...
        if isa(x, 'GeometricAlgebraFulcrumLib.Algebra.GeometricAlgebra.Float64.Multivectors.XGaFloat64Multivector') | isa(x, 'GeometricAlgebraFulcrumLib.Modeling.Geometry.CGa.Float64.Blades.CGaFloat64Blade')
            try
                str = char(GeometricAlgebraFulcrumLib.Modeling.Utilities.Text.TextUtils.CGaOrthoToText(x));
                fprintf('%s\n', str);
            catch
